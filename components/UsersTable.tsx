...
function getUserStatus(
  ...
  const hasOngoingClass = userSchedules.some((schedule) => {
    const start = timeToMinutes(schedule.time_start)
    const end = timeToMinutes(schedule.time_end)

    return now.minutes >= start && now.minutes < end
  })

  return hasOngoingClass ? 'active' : 'deactive'
}
...
export default function UsersTable({
  ...
  // Used to recalculate status every minute.
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((user) => {
      const status = getUserStatus(user.id, schedules)
